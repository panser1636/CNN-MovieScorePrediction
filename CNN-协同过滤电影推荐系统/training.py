...
def get_targets():
    targets = placeholder(tf.int32, [None, 1], name="targets")
    return targets
...
with train_graph.as_default():
    ...
    movie_id, movie_year, movie_categories, movie_titles, dropout_keep_prob = movie_nn.get_inputs()
    ...
    #  id embedding, genres embedding, title representation，然后得到整个movie的representation
    movie_id_embed_layer, movie_year_embed_layer = movie_nn.get_movie_id_year_embed_layer(movie_id, movie_year)
    ...
    _, movie_combine_layer_flat = movie_nn.get_movie_feature_layer(movie_id_embed_layer, movie_year_embed_layer,
...
# 训练网络
losses = {'train': [], 'test': [], 'prediction': [], 'y': []}
...
with tf.compat.v1.Session(graph=train_graph) as sess:
    ...
    # summaries for loss
    loss_summary = tf.compat.v1.summary.scalar('loss', loss)  # 输出包含单个标量值的 Summary 协议缓冲区
    ...
    inference_summary_op = tf.compat.v1.summary.merge([loss_summary])
    inference_summary_dir = os.path.join(out_dir, 'summaries', 'inference')
    inference_summary_writer = tf.compat.v1.summary.FileWriter(inference_summary_dir, sess.graph)
    ...
    for epoch_i in range(num_epochs):  # 5
        ...
        for batch_i in range(len(train_X) // batch_size):
            ...
            feed = {
                ...
                user_job: np.reshape(x.take(4, 1), [batch_size, 1]),  # user_job: np.zeros([batch_size, 1]),
                movie_id: np.reshape(x.take(1, 1), [batch_size, 1]),
                movie_year: np.reshape(x.take(7, 1), [batch_size, 1]),
            ...
            step, train_loss, summaries, _ = sess.run([global_step, loss, train_summary_op, train_op], feed_dict=feed)
            ...
            if batch_i % show_every_n_batches == 0:
                ...
                print('training prediction: %.2f,  expectation: %d' % (prediction[0][0], y[0][0]))
                ...
                print('{}: Epoch {:>3} Batch {:>4}/{}   train_loss = {:.3f}'.format(time_str,
                                                                                    ...
                                                                                    train_loss))
        if epoch_i + 1 - num_epochs == 0:
            cor_num_0p5 = 0
            cor_num_1 = 0
            cor_num_1p5 = 0
            cor_sum = 0
            # 使用测试数据集对本次的epoch训练（使用测试集全部数据，对模型进行一次完整训练）的模型进行测试
            for batch_i in range(len(test_X) // batch_size):
                ...
                feed = {
                    ...
                    movie_titles: titles,  # x.take(5,1)
                    targets: np.reshape(y, [batch_size, 1]),
                    dropout_keep_prob: 0.9,  # 1 不能跑
                }
                step, test_loss, summaries = sess.run([global_step, loss, inference_summary_op], feed_dict=feed)
                # step, test_loss, summaries, _ = sess.run([global_step, loss, inference_summary_op], feed_dict=feed)
                ...
                time_str = datetime.datetime.now().isoformat()
                prediction = inference.eval(feed)
                cor_sum += abs(prediction[0][0] - y[0][0])
                if abs(prediction[0][0] - y[0][0]) <= 0.5:
                    cor_num_0p5 += 1
                if abs(prediction[0][0] - y[0][0]) <= 1:
                    cor_num_1 += 1
                if abs(prediction[0][0] - y[0][0]) <= 1.5:
                    cor_num_1p5 += 1

                if batch_i % show_every_n_batches == 0:
                    losses['prediction'].append(prediction[0][0])
                    losses['y'].append(y[0][0])
            ...
            # print("误差小于0.5的比例为：", cor_num_0p5 / (len(test_X) // batch_size))
            print("误差小于1的比例为：", cor_num_1 / (len(test_X) // batch_size))
            print("误差小于1.5的比例为：", cor_num_1p5 / (len(test_X) // batch_size))
            print("平均误差为：", cor_sum / (len(test_X) // batch_size))
...
plt.figure(2)
plt.plot(losses['prediction'], color='r', linestyle='--', marker='*', label='prediction')
plt.plot(losses['y'], color='b', linestyle='--', marker="o", label='y')
plt.legend()
_ = plt.ylim()
