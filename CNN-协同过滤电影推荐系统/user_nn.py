...
features = pickle.load(open('./model/features.p', mode='rb'))
...
def get_inputs():
	...
	uid = placeholder(tf.int32, [None, 1], name='uid')
	user_gender = placeholder(tf.int32, [None, 1], name='user_gender')
	user_age = placeholder(tf.int32, [None, 1], name='user_age')
	user_job = placeholder(tf.int32, [None, 1], name='user_job')
	return uid, user_gender, user_age, user_job


def get_user_embedding(uid, user_gender, user_age, user_job):  # 参数列表删掉
	"""
	定义对user特征的embedding
	...
	with tf.name_scope('user_embedding'):
		uid_embed_matrix = tf.Variable(random_uniform([
			uid_max, embed_dim], -1, 1), name='uid_embed_matrix')
		uid_embed_layer = tf.nn.embedding_lookup(
			uid_embed_matrix, uid, name='uid_embed_layer')
		gender_embed_matrix = tf.Variable(random_uniform([
		...
		age_embed_matrix = tf.Variable(random_uniform([
		...
		job_embed_matrix = tf.Variable(random_uniform([
...
def get_user_feature_layer(
	...
	with tf.name_scope('user_fc'):
		...
		user_combine_layer = tf_slim.fully_connected(user_combine_layer, 1024, tf.nn.relu)
		user_combine_layer = tf_slim.fully_connected(user_combine_layer, user_feature_size, tf.nn.relu)  # (?, 1, 4*embed_dim) -->(?,1 , 512)
		user_combine_layer = tf_slim.fully_connected(user_combine_layer, 200, tf.nn.relu)
