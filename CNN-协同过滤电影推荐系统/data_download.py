...
#  hash_code = 'c4d9eecfca2ab87c1945afe126590906'
	if os.path.exists(save_path):
		print('{} is already exiting..'.format(data_name))  # 存在不下载
	else:
		with DLProgress(unit='B', unit_scale=True, miniters=1, desc='Downloading {}'.format(data_name)) as pbar:
			urlretrieve(url, save_path, pbar.hook)  # 不存在则下载文件， pbar.hook 回调函数，可以用来显示下载进度
